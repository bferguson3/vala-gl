...
static int main(string[] args)
{
    ...
    sp.SetVertexShape(posAttr, 2, GL_FLOAT, 5, 0); // configure the shader to use n(x,y)f format, 
    sp.SetVertexShape(colAttr, 3, GL_FLOAT, 5, 2);          //uses current VAO
    ...
    while(!myNewWindow.should_close)
    {
        ...
        float fade = (float)
            ...
            / 2.0f;
        sp.SetUniform("alpha", new Vector.1f(fade)); // set color 
